...
import { elemento_control_plagas_hallazgos } from "../functions/llavesControlDePlagas";
...
export default function TablaControlPlagasHallazgos ():JSX.Element {
    ...
    const obtenerData = async (): Promise<void> => {
        try {
            const request = {
                collection: 'users',
                action: 'get_control_plagas_hallazgos',
                page: page,
        ...
        <div className="filtroContainer">
            ...
            <label>
                ...
                <select onChange={(e):void => setArea(e.target.value)}>
                    ...
                    {Object.keys(elemento_control_plagas_hallazgos).map(llave => (
                        <option value={llave} key={llave}>{elemento_control_plagas_hallazgos[llave]}</option>
                    ))}
                </select>
            </label>
            <label>
            ...
            <tbody>
                {data.map((item, index) => (
                    <tr key={item.id} className={`${index % 2 === 0 ? 'fondo-par' : 'fondo-impar'}`}>
                        ...
                        <td>{elemento_control_plagas_hallazgos[item.elemento]}</td>
