  ...
  const datosFiltrados = props.data
    ...
    .filter((contenedor) => {
      return (
        ...
        (!props.filtroFechaEntrada ||
          new Date(contenedor.infoContenedor.fechaCreacion) >=
            (props.filtroFechaEntrada as Date)) &&
        (!props.filtroFechaSalida ||
          new Date(contenedor.infoContenedor.fechaSalida) <= (props.filtroFechaSalida as Date)) &&
        (!props.filtroFechaFinalizado ||
          new Date(contenedor.infoContenedor.fechaFinalizado) ===
            (props.filtroFechaFinalizado as Date))
      )
    })
      ...
      <table className={`mr-2 ml-2 w-full mt-4 border-2 table-fixed`}>
        <thead className={`${theme === 'Dark' ? 'bg-slate-700' : 'bg-slate-200'}`}>
          <tr className="h-14 broder-2">
            <th className={`${theme === 'Dark' ? 'text-white' : 'text-black'} text-sm`}>
              Contenedor
            </th>
        ...
        <tbody>
          {Array.isArray(datosFiltrados) &&
            datosFiltrados.map((contenedor, index) => (
              <tr className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-gray-200'}`} key={index}>
                <td className="p-2 text-sm  text-center">{contenedor._id}</td>
                <td className="p-2 text-sm  text-center">
                  {contenedor.infoContenedor.nombreCliente}
                </td>
                <td className="p-2 text-sm  text-center">
                  {formatFecha(contenedor.infoContenedor.fechaCreacion)}
                </td>
                <td className="p-2 text-sm  text-center">
                  {formatFecha(contenedor.infoContenedor.fechaFinalizado)}
                </td>
                <td className="p-2 text-sm  text-center">
                  {formatFecha(contenedor.infoContenedor.fechaSalida)}
                </td>
                ...
                <td className="p-2 text-sm  text-center">
                  <div className="flex justify-center items-center">
                    {contenedor.infoContenedor.urlInforme ? (
                      <a
                        href={contenedor.infoContenedor.urlInforme}
                        target="_blank"
                ...
                <td className="p-2 text-sm">
                  <div className="flex justify-center items-center">
                    {contenedor.infoContenedor.cerrado ? <FcOk /> : <FcCancel />}
                  </div>
                </td>
                <td className="p-2 text-sm">
                  <div className="flex justify-center items-center">
                    {contenedor.infoContenedor.desverdizado ? <FcOk /> : <FcCancel />}
                  </div>
                </td>
              </tr>
            ))}
