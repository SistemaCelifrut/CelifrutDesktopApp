...
export default function HistorialProcesado(props: propsType) {
  ...
  useEffect(() => {
    const asyncFunction = async () => {
      try {
        const request = { action: 'obtenerHistorialProceso2' }
  ...
  return (
    <div>
      ...
      {showModal &&
          createPortal(
            <ModificarHistorialProceso closeModal={closeModal} propsModal={propsModal} theme={props.theme} />,
            document.body
          )}
    </div>
  )
