...
export default function ModificarHistorialDirecto(props: vaciadoType): JSX.Element {
  ...
  const vaciar = async (): Promise<void> => {
    try {
      ...
      const propsCanastillasInt = props.propsModal.documento.directoNacional / props.propsModal.documento.promedio
      ...
      if (canastillasInt > propsCanastillasInt) {
        ...
        new_lote.directoNacional -= canastillasInt * new_lote.promedio;
        ...
        const request = {
          ...
          action: 'putLotes',
          query: 'proceso',
          record: 'modificarHistorialDirectoNacional'
        }
        const response = await window.api.server(request)
        if (response.status === 200) {
          props.closeModal()
          props.setShowSuccess(true)
          props.setMessage("Historial modificado con exito!")
          setInterval(() => {
