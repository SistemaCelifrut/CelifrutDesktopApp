...
const MiComponente: React.FC = () => {
  ...
  const handleDropOrdenVaceo = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (draggedLote && draggedLote.predio !== null) {
      ...
      const existingLoteIndex = newLotesOrdenVaceo.findIndex(
        (lote) => lote.lote._id === draggedLote._id
      )
  ...
  const handleDropLote = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    setLotes([
      ...lotes.slice(0, dropIndex),
      { ...draggedLote.lote, ordenVaceo: undefined },
      ...lotes.slice(dropIndex)
    ])
  }
  ...
  return (
    <div className="flex justify-center items-start">
      <div className="w-1/2 p-4">
        <h1 className={`${theme === 'Dark' ? 'text-white' : 'text-black'} text-lg font-bold mb-4`}>
          Listado de Predios
        </h1>
          ...
          {lotes.map((lote, index) => {
            ...
            return (
              <div
                key={index}
                onClick={() => {}}
                style={{ display: showPredios[lote._id] !== false ? 'block' : 'none' }}
                className={`p-2 rounded-md cursor-move mb-2 border border-gray-200 shadow-lg hover:shadow-xl transition duration-300 ${
                  isOrdenVaceo ? 'bg-green-200' : isDesverdizado ? 'bg-orange-200' : 'bg-gray-100'
                }`}
                draggable
                onDragStart={(e) => handleDragStartLote(e, index)}
                onDragOver={handleDragOverLote}
                onDrop={handleDropLote}
              >
                ...
                {lote.desverdizado && (
                  <div>
                    <p className="text-sm">
                      <FontAwesomeIcon icon={faBox} className="mr-1" />
                      Canastillas: {lote.desverdizado.canastillas}
                    </p>
                  </div>
                )}
              </div>
      ...
      <div
        className="w-1/2 p-4 border border-gray-300 rounded-md max-h-96 overflow-y-auto drop-zone"
        style={{
        ...
        onDragOver={handleDragOverOrdenVaceo}
        onDrop={handleDropOrdenVaceo}
      >
        ...
        <ul className="space-y-4">
          {lotesOrdenVaceo.map((loteOrdenVaceo, index) => (
            <div
              key={index}
              onClick={() => {}}
              className="bg-blue-100 p-2 rounded-md cursor-move mb-2 relative border border-gray-200 shadow-lg hover:shadow-xl transition duration-300"
              draggable
              onDragStart={(e) => handleDragStartOrdenVaceo(e, index)}
            >
              <div
                className="absolute top-0 left-0 bg-green-500 text-white rounded-full h-6 w-6 flex items-center justify-center"
                style={{ opacity: 0.7 }}
              >
                {index + 1}
              </div>
              <div
                ...
                onClick={() => handleRemoveFromVaceo(index)}
              >
                X
              </div>
              ...
              <p className="mb-1 text-sm">
                Fecha de Ingreso:{' '}
                {loteOrdenVaceo.lote.fechaIngreso
                  ? format(new Date(loteOrdenVaceo.lote.fechaIngreso), 'dd/MM/yyyy')
                  : '-'}
              </p>
              <p className="mb-1 text-sm">Tipo de Fruta: {loteOrdenVaceo.lote.tipoFruta}</p>
              <p className="text-sm">
                Canastillas: {loteOrdenVaceo.lote.inventarioActual?.inventario}
              </p>
            </div>
          ))}
        </ul>
